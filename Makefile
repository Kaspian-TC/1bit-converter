...
PNG_NAME = anime_image
1BIT_VER = .1
1bit_converter.exe: main_code.c imgutils.o 1bit_funcs.o
	$(CC) $(CFLAGS) -o $@ main_code.c imgutils.o 1bit_funcs.o
...
png: 1bit_converter.exe
	ffmpeg -i "PNG images"/$(PNG_NAME).png "ppm images"/$(PNG_NAME).ppm -loglevel quiet -y
	$^ -i "ppm images"/$(PNG_NAME).ppm "1bit images"/$(PNG_NAME).1bit -c $(1BIT_VER) -d
